...
void setup() {
  ...
  wristServo.write(0);
  gripperServo.write(0);  // Closed position
...
void loop() {
  if (Serial.available() > 0) {
    ...
    // Robot Arm Commands (e.g., "B 90" to move base to 90°)
    else {
      char servoType = command.charAt(0);
      int angle = command.substring(2).toInt();

      switch (servoType) {
        case 'B': baseServo.write(angle); Serial.println("Base moved"); break;
